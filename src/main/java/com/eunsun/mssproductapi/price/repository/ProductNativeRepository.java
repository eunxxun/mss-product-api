package com.eunsun.mssproductapi.price.repository;

import com.eunsun.mssproductapi.price.dto.LowestPriceBrand;
import com.eunsun.mssproductapi.price.dto.BrandPriceRange;
import com.eunsun.mssproductapi.price.dto.LowestPriceCategory;
import com.eunsun.mssproductapi.price.dto.CategoryPrice;
...
public interface ProductNativeRepository extends JpaRepository<Product, Long> {
    @Query(value = "SELECT category_nm as categoryNm, brand_nm as brandNm, price FROM " +
            ...
            "WHERE subquery.rn = 1",
            nativeQuery = true)
    List<LowestPriceCategory> findCategoryMinPrices();
    ...
    @Query(value = "SELECT brand_id as brandId, brand_nm as brandNm, SUM(price) AS totalPrice " +
            ...
            "LIMIT 1",
            nativeQuery = true)
    LowestPriceBrand findLowestPriceBrandWithTotal();

    @Query("SELECT new com.eunsun.mssproductapi.price.dto.CategoryPrice(c.categoryNm, MIN(p.price)) " +
            "FROM Product p " +
            "JOIN p.category c " +
            "JOIN p.brand b " +
            "WHERE b.id = :brandId " +
    ...
    @Query(value = "SELECT " +
            ...
            "WHERE c.category_nm = :categoryNm " +
            "AND p_min.price = (SELECT MIN(p2.price) " +
            "                   FROM product p2 " +
            "                   JOIN category c2 ON p2.category_id = c2.id " +
            "                   WHERE c2.category_nm = :categoryNm AND p2.del_yn = false) " +
            "AND p_max.price = (SELECT MAX(p3.price) " +
            "                   FROM product p3 " +
            "                   JOIN category c3 ON p3.category_id = c3.id " +
            "                   WHERE c3.category_nm = :categoryNm AND p3.del_yn = false) " +
            "AND p_min.del_yn = false " +
            "AND p_max.del_yn = false",
            nativeQuery = true)
    BrandPriceRange findBrandPriceRangeByCategoryNm(@Param("categoryNm") String categoryNm);
}
