...
- 대량의 상품 데이터에 대비한 조회 성능 개선
  - 조회 성능을 향상 시키기 위해 Redis Cache 사용을 고민했으나 상품 데이터의 변경이 많을 것으로 예상되어 이 내용은 삭제했습니다.
  - Kafka 같은 메시지 큐를 사용하여 상품 가격 변동 이벤트를 발행하고 발행된 이벤트는 컨슘하여 최저가, 최고가 테이블에 반영한 후 가격조회는 해당 테이블에서만 읽게 하는 방법도 좋을 것이라 생각합니다.
  - 현재 구조에서는 가격 조회에 필요한 인덱스를 최적화하여 조회 성능을 개선했습니다.
